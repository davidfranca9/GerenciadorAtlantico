from ..shared import *
from .ocr import _find_prod_table, _label_key_from_text
from .relatorios import _clean, _format_peso, formatar_moeda_brasileira
...
def _find_header_index(headers, *candidates):
    normalized = {_normalize_header_name(candidate) for candidate in candidates}
    for idx, header in enumerate(headers, start=1):
        if _normalize_header_name(header) in normalized:
            return idx
...
def _format_autorizacao_sheet(ws, headers, total_row=AUTORIZACAO_TOTAL_ROW):
    ...
    for col_idx, header in enumerate(headers, start=1):
        ...
        width = {
            ...
            "Placa cavalo mecÃ¢nico": 18,
            "Nome do condutor": 28,
            "Número do pedido": 16,
            "NÃºmero do pedido": 16,
            "Produto": 34,
            "Embalagem": 16,
            "Quantidade": 14,
            "Cidade/UF": 22,
...
def _write_autorizacao_rows(ws, headers, produtos, data_carregamento, nome_condutor=None, placa_cavalo=None):
    ...
    for idx, p in enumerate((produtos or [])[:max_items]):
        ...
        row_map = {
            ...
            "Placa cavalo mecÃ¢nico": placa_cavalo,
            "Nome do condutor": nome_condutor,
            "Número do pedido": int(contrato) if contrato.isdigit() else contrato,
            "NÃºmero do pedido": int(contrato) if contrato.isdigit() else contrato,
...
def append_rows_to_excel(excel_path, produtos, data_carregamento):
    ...
    _write_autorizacao_rows(ws, headers, produtos, data_carregamento)
    ...
    for idx, p in enumerate(produtos):
        ...
        row_map = {
            "Cliente": p.get("cliente"), "Data de Carregamento": data_carregamento,
            "Placa cavalo mecânico": None, "Nome do condutor": None,
            "Número do pedido": int(p.get("contrato")) if p.get("contrato") else None,
            "Produto": p.get("produto"), "Embalagem": p.get("embalagem"),
            "Quantidade": str(p.get("toneladas")).replace(".", ","),
            "Cidade/UF": p.get("cidade")
        }
        for col_idx, header in enumerate(headers, start=1):
...
def update_excel_with_driver_data(excel_path, driver_name, plate1, products_to_update):
    ...
    headers = get_headers_from_sheet(ws)
    try:
        contract_col_idx = _find_header_index(headers, "Número do pedido", "NÃºmero do pedido", "NÃƒÂºmero do pedido")
        driver_col_idx = _find_header_index(headers, "Nome do condutor")
        plate_col_idx = _find_header_index(headers, "Placa cavalo mecânico", "Placa cavalo mecÃ¢nico", "Placa cavalo mecÃƒÂ¢nico")
    except ValueError as e:
        messagebox.showerror("Erro de Coluna", f"Não foi possível encontrar a coluna '{e}' na planilha.")
        return
    for row in ws.iter_rows(min_row=2):
        cell_contract = ws.cell(row=row[0].row, column=contract_col_idx)
        if cell_contract.value is None:
            continue
...
def criar_planilha_especifica_motorista_data(novo_caminho_excel, produtos, data_carregamento, nome_condutor, placa_cavalo):
    ...
    for idx, p in enumerate(produtos):
        row_map = {
            ...
            "Placa cavalo mecÃ¢nico": placa_cavalo,
            "Nome do condutor": nome_condutor,
            "NÃºmero do pedido": p.get("contrato"),
            "Produto": p.get("produto"),
            "Embalagem": p.get("embalagem"),
            "Quantidade": p.get("toneladas"),
            "Cidade/UF": p.get("cidade")
        }
        for col_idx, header in enumerate(headers, start=1):
            ws.cell(row=start_row + idx, column=col_idx).value = row_map.get(header)
...
def criar_planilha_especifica_motorista(novo_caminho_excel, produtos, data_carregamento, nome_condutor, placa_cavalo):
    try:
        ...
        messagebox.showerror("Erro", f"Arquivo modelo de planilha '{EXCEL_FILE}' nÃ£o encontrado."); return
    except Exception as e:
        messagebox.showerror("Erro", f"NÃ£o foi possÃ­vel abrir a planilha modelo '{EXCEL_FILE}'.\n\nDetalhe: {e}"); return
    try:
        wb = load_workbook(EXCEL_FILE)
        ws = wb[SHEET_NAME]
    except FileNotFoundError:
    ...
    for idx, p in enumerate(produtos):
        row_map = {
            "Cliente": p.get("cliente"), "Data de Carregamento": data_carregamento,
            "Placa cavalo mecânico": placa_cavalo, "Nome do condutor": nome_condutor,
            "Número do pedido": p.get("contrato"), "Produto": p.get("produto"),
            "Embalagem": p.get("embalagem"), "Quantidade": p.get("toneladas"),
            "Cidade/UF": p.get("cidade")
        }
        for col_idx, header in enumerate(headers, start=1):
            ws.cell(row=start_row + idx, column=col_idx).value = row_map.get(header)
...
def fill_carta_frete_docx(doc, dados):
    ...
    mapa_campos_normais = {
        ...
        "PLACA_CAVALO": "PLACA CAVALO:", "AUTORIZACAO_NUM": "AUTORIZAÇÃO Nº:"
    }
